 ...
 * Caches result
 */
const findPkgDir = lodash_1.default.memoize(package_directory_1.packageDirectory, (opts) => opts?.cwd);
/**
...
async function _readPkgJson(cwd, normalize) {
    ...
    if (normalize) {
        const pkg = await (0, read_pkg_1.readPackage)({ cwd: pkgDir, normalize });
        return { pkg, pkgPath };
    }
    else {
        const pkg = await (0, read_pkg_1.readPackage)({ cwd: pkgDir });
        return { pkg, pkgPath };
    }
}
/**
