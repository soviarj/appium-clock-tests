...
async function getProcessIds(appName) {
    ...
    try {
        ...
        const code = err.code;
        if (code !== 1) {
            throw new Error(`Error getting process ids for app '${appName}': ${err.message}`, { cause: err });
        }
        pids = [];
    }
    return pids;
...
async function killProcess(appName, force = false) {
    ...
    try {
        ...
        const code = err.code;
        if (code !== 1) {
            throw new Error(`Error killing app '${appName}' with pkill: ${err.message}`, { cause: err });
        }
    }
}
