...
async function resolveLaunchableActivity(pkg, opts = {}) {
    ...
    if (!preferCmd || (await this.getApiLevel()) < 24) {
        ...
        if (support_1.util.isEmpty(names)) {
...
async function grantAllPermissions(pkg, apk) {
    ...
    if (apiLevel >= MIN_API_LEVEL_WITH_PERMS_SUPPORT &&
        ...
        if (support_1.util.isEmpty(permissionsToGrant)) {
...
async function getPackageInfo(pkg) {
    ...
    const installedPattern = new RegExp(`^\\s*Package\\s+\\[${support_1.util.escapeRegExp(pkg)}\\][^:]+:$`, 'm');
...
async function listInstalledPackages(opts = {}) {
    ...
    try {
        ...
        if ((error.stderr || error.stdout || error.message)?.includes('access user') &&
            support_1.util.isEmpty(user)) {
...
async function startApp(startAppOptions) {
    ...
    try {
        ...
        throw new Error(`Cannot start the '${appDescriptor}' application. ` +
            ...
            `Original error: ${error.message}`, { cause: e });
    }
}
/**
...
async function getFocusedPackageAndActivity() {
    ...
    try {
        ...
        const error = e;
        throw new Error(`Could not retrieve the currently focused package and activity. Original error: ${error.message}`, { cause: e });
    }
    ...
    if (focusedAppCandidates.length > 1 && currentFocusAppCandidates.length > 0) {
        ...
        return ((0, utils_1.intersectionWith)(focusedAppCandidates, currentFocusAppCandidates, (value, other) => {
...
async function waitForActivityOrNot(pkg, activity, waitForStop, waitMs = 20000) {
    ...
    const splitNames = (names) => names.split(',').map((x) => x.trim());
    ...
    const conditionFunc = async () => {
        ...
        if (appActivity && appPackage) {
            ...
            const isFound = allPackages.includes(appPackage) &&
...
function parseLaunchableActivityNames(dumpsys) {
    const mainActivityNameRe = new RegExp(`^\\s*${support_1.util.escapeRegExp(MAIN_ACTION)}:$`);
    ...
    for (const line of dumpsys.split('\n').map((x) => x.trimEnd())) {
        ...
        if (mainActivityNameRe.test(line)) {
            ...
            if (!support_1.util.isEmpty(block)) {
                blocks.push(block);
                block = [];
            }
            continue;
...
function extractMatchingPermissions(dumpsysOutput, groupNames, grantedState = null) {
    const groupPatternByName = (groupName) => new RegExp(`^(\\s*${support_1.util.escapeRegExp(groupName)} permissions:[\\s\\S]+)`, 'm');
...
async function listAppProcessIds(pkg) {
    ...
    const pidRegex = new RegExp(`ProcessRecord\\{[\\w]+\\s+(\\d+):${support_1.util.escapeRegExp(pkg)}\\/`);
    ...
    return support_1.util.uniq(pids);
}
/**
...
async function isAppRunning(pkg) {
    return !support_1.util.isEmpty(await this.listAppProcessIds(pkg));
}
/**
