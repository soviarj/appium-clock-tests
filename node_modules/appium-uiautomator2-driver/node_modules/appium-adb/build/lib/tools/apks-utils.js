...
const APKS_INSTALL_TIMEOUT = utils_1.APK_INSTALL_TIMEOUT * 2;
process.on('exit', () => {
    if (!APKS_CACHE.size) {
        return;
    }
...
async function execBundletool(args, errorMsg) {
    ...
    let stdout;
    try {
        ({ stdout } = await (0, teen_process_1.exec)(await (0, utils_1.getJavaForOs)(), args, {
            env,
            timeout: BUNDLETOOL_TIMEOUT_MS,
        }));
        logger_1.log.debug(`Command stdout: ${support_1.util.truncateString(stdout, { length: 300 })}`);
        return stdout;
    }
    catch (e) {
        const err = e;
...
async function installMultipleApks(apkPathsToInstall, options = {}) {
    const installArgs = (0, utils_1.buildInstallArgs)(await this.getApiLevel(), options);
...
async function extractFromApks(apks, dstPath) {
    ...
    return await APKS_CACHE_GUARD.acquire(apks, async () => {
        ...
        await (0, utils_1.unzipFile)(apks, tmpRoot);
