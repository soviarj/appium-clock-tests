...
async function getDeviceProperty(property) {
    ...
    logger_1.log.debug(`Current device property '${property}': ${val}`);
    return val;
}
/**
...
async function setDeviceProperty(prop, val, opts = {}) {
    ...
    logger_1.log.debug(`Setting device property '${prop}' to '${val}'`);
...
async function getTimeZone() {
    logger_1.log.debug('Getting current timezone');
    try {
...
async function getPlatformVersion() {
    logger_1.log.info('Getting device platform version');
    try {
...
async function setHiddenApiPolicy(value, ignoreError = false) {
    try {
        ...
        logger_1.log.info(`Failed to set setting keys '${HIDDEN_API_POLICY_KEYS}' to '${value}'. ` +
...
async function setDefaultHiddenApiPolicy(ignoreError = false) {
    try {
        ...
        logger_1.log.info(`Failed to delete keys '${HIDDEN_API_POLICY_KEYS}'. Original error: ${err.message}`);
    }
}
/**
...
async function ensureCurrentLocale(language, country, script) {
    ...
    if (!hasLanguage && !hasCountry) {
        logger_1.log.warn('ensureCurrentLocale requires language or country');
        return false;
    }
    ...
    return ((await (0, asyncbox_1.retryInterval)(5, 1000, async () => {
        if (apiLevel < 23) {
            logger_1.log.debug(`Requested locale: ${lcLanguage}-${lcCountry}`);
            let actualLanguage;
            if (hasLanguage) {
                actualLanguage = (await this.getDeviceLanguage()).toLowerCase();
                logger_1.log.debug(`Actual language: ${actualLanguage}`);
            ...
            if (hasCountry) {
                ...
                logger_1.log.debug(`Actual country: ${actualCountry}`);
        ...
        logger_1.log.debug(`Requested locale: ${expectedLocale}. Actual locale: '${actualLocale}'`);
        const languagePattern = `^${support_1.util.escapeRegExp(lcLanguage)}-${script ? support_1.util.escapeRegExp(script) + '-' : ''}`;
        const checkLocalePattern = (p) => new RegExp(p, 'i').test(actualLocale);
        if (hasLanguage && !hasCountry) {
            return checkLocalePattern(languagePattern);
        }
        const countryPattern = `${script ? '-' + support_1.util.escapeRegExp(script) : ''}-${support_1.util.escapeRegExp(lcCountry)}$`;
        if (!hasLanguage && hasCountry) {
...
async function getDeviceIdleWhitelist() {
    ...
    logger_1.log.info('Listing packages in Doze whitelist');
...
async function addToDeviceIdleWhitelist(...packages) {
    if (support_1.util.isEmpty(packages) || (await this.getApiLevel()) < 23) {
        // Doze mode has only been added since Android 6
        return false;
    }
    logger_1.log.info(`Adding ${support_1.util.pluralize('package', packages.length)} ${JSON.stringify(packages)} to Doze whitelist`);
...
async function isAnimationOn() {
    return (await Promise.all(ANIMATION_SCALE_KEYS.map(async (k) => (await this.getSetting('global', k)) !== '0.0'))).includes(true);
}
/**
