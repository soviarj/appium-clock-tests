...
function getSdkBinaryLocationCandidates(sdkRoot, fullBinaryName) {
    return SDK_BINARY_ROOTS.map((x) => node_path_1.default.resolve(sdkRoot, ...(Array.isArray(x) ? x : [x]), fullBinaryName));
}
/**
...
exports.getBinaryNameForOS = (0, utils_1.memoize)(_getBinaryNameForOS);
/**
...
async function getBinaryFromSdkRoot(binaryName) {
    ...
    if (this.buildToolsVersion) {
        ...
        if (support_1.util.isEmpty(buildToolsDirs)) {
    ...
    binaryLocs.push(...buildToolsDirs
        .map((dir) => [node_path_1.default.resolve(dir, fullBinaryName), node_path_1.default.resolve(dir, 'lib', fullBinaryName)])
        .flat());
    let binaryLoc = null;
...
async function getAndroidBinaryPath(binaryName) {
    ...
    const sdkRoot = (0, utils_1.getSdkRootFromEnv)();
...
async function getConnectedDevices(opts = {}) {
    ...
    if (support_1.util.isEmpty(devices)) {
 ...
 */
exports.resetTelnetAuthToken = (0, utils_1.memoize)(async function resetTelnetAuthToken() {
...
async function adbExec(cmd, opts) {
    ...
    const optsCopy = (0, utils_1.cloneDeep)(opts ?? {});
    // setting default timeout for each command to prevent infinite wait.
    optsCopy.timeout = optsCopy.timeout || this.adbExecTimeout || utils_1.DEFAULT_ADB_EXEC_TIMEOUT;
...
async function launchAVD(avdName, opts = {}) {
    ...
    if (!support_1.util.isEmpty(args)) {
        launchArgs.push(...(Array.isArray(args) ? args : support_1.util.shellParse(`${args}`)));
    }
    logger_1.log.debug(`Running '${emulatorBinaryPath}' with args: ${support_1.util.quote(launchArgs)}`);
    if (!support_1.util.isEmpty(env)) {
    ...
    if (isDelayAdbFeatureEnabled) {
        try {
            ...
            const error = e;
            throw new Error(`'${processedAvdName}' Emulator has failed to boot: ${error.stderr || error.message}`, { cause: e });
        }
    }
 ...
 */
exports.getVersion = (0, utils_1.memoize)(async function getVersion() {
    let stdout;
    try {
...
async function waitForEmulatorReady(timeoutMs = 20000) {
    ...
    try {
        ...
        if (servicesValue) {
            const missingServices = (0, utils_1.zip)(REQUIRED_SERVICES, requiredServicesRe)
...
async function reboot(retries = DEFAULT_ADB_REBOOT_RETRIES) {
    ...
    try {
        ...
        await (0, asyncbox_1.sleep)(2000); // let the emu finish stopping;
        ...
        if (message.includes('must be root')) {
            throw new Error(`Could not reboot device. Rebooting requires root access and ` +
                `attempt to get root access on device failed with error: '${message}'`, { cause: e });
        }
        throw error;
    }
    finally {
...
async function installMitmCertificate(cert) {
    ...
    try {
        ...
        throw new Error(`Cannot inject the custom certificate. ` +
            ...
            `Original error: ${error.message}`, { cause: err });
    }
    finally {
...
async function shellChunks(argTransformer, args) {
    ...
    if (!support_1.util.isEmpty(cmdChunk)) {
        commands.push(cmdChunk);
    }
 ...
 */
exports.getBuildToolsDirs = (0, utils_1.memoize)(async function getBuildToolsDirs(sdkRoot) {
    ...
    try {
        ...
        const pairs = await (0, asyncbox_1.asyncmap)(buildToolsDirs, async (dir) => [(await support_1.fs.stat(dir)).mtime.valueOf(), dir]);
