...
async function executeApksigner(args) {
    ...
    const fullCmd = [await (0, utils_1.getJavaForOs)(), '-Xmx1024M', '-Xss1m', '-jar', apkSignerJar, ...args];
...
async function signWithDefaultCert(apk) {
    ...
    const args = [
        'sign',
        '--key',
        await (0, utils_1.getResourcePath)(DEFAULT_PRIVATE_KEY),
        '--cert',
        await (0, utils_1.getResourcePath)(DEFAULT_CERTIFICATE),
        apk,
    ];
    try {
        ...
        throw new Error(`Could not sign '${apk}' with the default certificate. ` +
            `Original error: ${err.stderr || err.stdout || err.message}`, { cause: e });
    }
}
/**
...
async function signWithCustomCert(apk) {
    ...
    try {
        ...
        try {
            ...
            const jarsigner = node_path_1.default.resolve(await (0, utils_1.getJavaHome)(), 'bin', `jarsigner${support_1.system.isWindows() ? '.exe' : ''}`);
            const fullCmd = [
                jarsigner,
                '-sigalg',
                'MD5withRSA',
            ...
            throw new Error(`Could not sign with custom certificate. ` +
                `Original error: ${execErr.stderr || execErr.message}`, { cause: e });
        }
    }
}
/**
...
async function sign(appPath) {
    if (appPath.endsWith(utils_1.APKS_EXTENSION)) {
...
async function zipAlignApk(apk) {
    ...
    try {
        ...
        throw new Error(`zipAlignApk failed. Original error: ${err.message || err.stderr}`, { cause: e });
    }
}
/**
...
async function checkApkCert(appPath, pkg, opts = {}) {
    ...
    if (node_path_1.default.extname(appPath) === utils_1.APKS_EXTENSION) {
...
async function getKeystoreHash() {
    ...
    const keytool = node_path_1.default.resolve(await (0, utils_1.getJavaHome)(), 'bin', `keytool${support_1.system.isWindows() ? '.exe' : ''}`);
    ...
    try {
        ...
        const result = {};
        const hashNames = [SHA512, SHA256, SHA1, MD5];
        ...
        if (support_1.util.isEmpty(result)) {
        ...
        throw new Error(`Cannot get the hash of '${this.keystorePath}' keystore. ` +
            `Original error: ${err.stderr || err.message}`, { cause: e });
    }
}
