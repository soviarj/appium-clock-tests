...
async function packageAndLaunchActivityFromManifest(appPath) {
    ...
    const { name: apkPackage, launchableActivity: { name: apkActivity }, } = await utils_1.readPackageManifest.bind(this)(appPath);
...
async function targetSdkVersionFromManifest(appPath) {
    ...
    const { targetSdkVersion } = await utils_1.readPackageManifest.bind(this)(appPath);
    if (!targetSdkVersion) {
...
async function compileManifest(manifest, manifestPackage, targetPackage) {
    ...
    try {
        ...
        try {
            ...
            throw new Error(`Cannot compile the manifest. Original error: ${e1.message || e1.stderr}`, { cause: e1 });
        }
    }
...
async function insertManifest(manifest, srcApk, dstApk) {
    ...
    await (0, utils_1.unzipFile)(`${manifest}.apk`);
...
async function hasInternetPermissionFromManifest(appPath) {
    ...
    const { usesPermissions } = await utils_1.readPackageManifest.bind(this)(appPath);
...
async function getAndroidPlatformAndPath(sdkRoot) {
    ...
    if (support_1.util.isEmpty(platformsMapping)) {
