...
async function powerAC(state = 'on') {
    if (!Object.values(this.POWER_AC_STATES).includes(state)) {
...
async function sensorSet(sensor, value) {
    if (!Object.values(this.SENSORS).includes(sensor)) {
...
async function sendSMS(phoneNumber, message = '') {
    ...
    if (!Number.isInteger(phoneNumber) && support_1.util.isEmpty(phoneNumber)) {
...
async function gsmCall(phoneNumber, action) {
    if (!Object.values(this.GSM_CALL_ACTIONS).includes(action)) {
        throw new TypeError(`Invalid gsm action param ${action}. Supported values: ${Object.values(this.GSM_CALL_ACTIONS)}`);
    }
    if (!Number.isInteger(phoneNumber) && support_1.util.isEmpty(phoneNumber)) {
        throw new TypeError('Phone number most not be empty');
    }
...
async function gsmSignal(strength = 4) {
    ...
    if (!this.GSM_SIGNAL_STRENGTHS.includes(strengthInt)) {
        throw new TypeError(`Invalid signal strength param ${strength}. Supported values: ${Object.values(this.GSM_SIGNAL_STRENGTHS)}`);
    }
...
async function gsmVoice(state = 'on') {
    ...
    if (!Object.values(this.GSM_VOICE_STATES).includes(state)) {
        throw new TypeError(`Invalid gsm voice state param ${state}. Supported values: ${Object.values(this.GSM_VOICE_STATES)}`);
    }
        ...
        client.on('data', (chunk) => {
            ...
            if (okFlag.test(output)) {
                ...
                if (!isCommandSent) {
                    ...
                    serverResponse = [];
                    const cmdStr = Array.isArray(cmd) ? support_1.util.quote(cmd) : `${cmd}`;
