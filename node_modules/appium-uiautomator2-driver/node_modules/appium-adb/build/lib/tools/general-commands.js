...
async function getApiLevel() {
    ...
    try {
        ...
        if (apiLevelDiff >= 0 && (await this.getPlatformVersion()).toLowerCase() === codename) {
            logger_1.log.debug(`Release version is ${codename.toUpperCase()} but found API Level ${apiLevel}. Setting API Level to ${apiLevel + 1}`);
            apiLevel++;
        }
        this._apiLevel = apiLevel;
        logger_1.log.debug(`Device API level: ${this._apiLevel}`);
...
async function clearTextField(length = 100) {
    ...
    logger_1.log.debug(`Clearing up to ${length} characters`);
    if (length === 0) {
        return;
    }
 ...
 */
async function back() {
    logger_1.log.debug('Pressing the BACK button');
...
async function goToHome() {
    logger_1.log.debug('Pressing the HOME button');
...
function screenrecord(destination, options = {}) {
    ...
    logger_1.log.debug(`Building screenrecord process with the command line: adb ${support_1.util.quote(fullCmd)}`);
...
async function listFeatures() {
    this._memoizedFeatures =
        this._memoizedFeatures ||
            (0, utils_1.memoize)(async () => await this.adbExec(['features']), () => this.curDeviceId);
    try {
...
async function takeScreenshot(displayId) {
    ...
    try {
        ...
        throw new Error(`Screenshot of the ${displayDescr} failed. ` +
            `Code: '${err.code ?? 'unknown'}', output: '${outputStr}'`, { cause: e });
    }
    if (stdout.length === 0) {
