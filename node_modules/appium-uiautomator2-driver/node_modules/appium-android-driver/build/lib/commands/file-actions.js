...
async function pullFile(remotePath) {
    ...
    if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
        ...
        tmpDestination = `/data/local/tmp/${node_path_1.default.posix.basename(pathInContainer)}`;
        try {
        ...
        if (remotePath.startsWith(CONTAINER_PATH_MARKER)) {
            ...
            try {
                await this.adb.shell([
                    'run-as',
                    packageId,
                    `mkdir -p '${escapePath(node_path_1.default.posix.dirname(pathInContainer))}'`,
                ]);
...
function parseContainerPath(remotePath) {
    ...
    return [match[1], node_path_1.default.posix.resolve(`/data/data/${match[1]}`, match[2])];
}
/**
