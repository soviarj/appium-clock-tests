...
async function mobileStartScreenStreaming(width, height, bitRate, host = DEFAULT_HOST, port = DEFAULT_PORT, pathname, tcpPort = DEFAULT_PORT + 1, quality = DEFAULT_QUALITY, considerRotation = false, logPipelineDetails = false) {
    ...
    let mjpegSocket;
    let mjpegServer;
    let mjpegConnectTimeoutId;
    const mjpegConnectTimeoutPromise = new Promise((_resolve, reject) => {
...
async function initDeviceStreamingProc(adb, log, deviceInfo, opts = {}) {
    ...
    try {
        ...
        throw log.errorWithException(`Cannot start the screen streaming process. Original error: ${e.message}`);
    }
    finally {
...
async function initGstreamerPipeline(deviceStreamingProc, deviceInfo, log, opts) {
    ...
    try {
        ...
        didFail = true;
        throw log.errorWithException(`Cannot start the screen streaming pipeline. Original error: ${e.message}`);
    }
    finally {
...
function extractSafePathnameFromUrl(requestUrl) {
    try {
        return new node_url_1.default.URL(String(requestUrl ?? '/'), 'http://127.0.0.1').pathname;
    }
    catch {
        return '/';
    }
