...
async function mobileChangePermissions(permissions, appPackage, action, target = PERMISSION_TARGET.PM) {
    ...
    if (!appPackage) {
        throw new driver_1.errors.InvalidArgumentError(`A valid appPackage must be provided`);
    }
    action ??= lodash_1.default.toLower(target) === PERMISSION_TARGET.APPOPS ? APPOPS_ACTION.ALLOW : PM_ACTION.GRANT;
