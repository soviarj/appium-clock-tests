...
async function startChromedriverProxy(context, webviewsMapping) {
    ...
    if (this.sessionChromedrivers[context]) {
        ...
        cd.on(appium_chromedriver_1.Chromedriver.EVENT_CHANGED, async (msg) => {
            if (msg.state === appium_chromedriver_1.Chromedriver.STATE_STOPPED) {
                try {
                    await this.onChromedriverStop(context);
                }
                catch (err) {
                    this.log.warn(`Error handling chromedriver stop event for context ${context}: ${err.message}`);
                }
            }
        });
...
async function startChromeSession() {
    ...
    chromedriver.on(appium_chromedriver_1.Chromedriver.EVENT_CHANGED, async (msg) => {
        if (msg.state === appium_chromedriver_1.Chromedriver.STATE_STOPPED) {
            try {
                await this.onChromedriverStop(helpers_1.CHROMIUM_WIN);
            }
            catch (err) {
                this.log.warn(`Error handling chromedriver stop event for context ${helpers_1.CHROMIUM_WIN}: ${err.message}`);
            }
        }
    });
