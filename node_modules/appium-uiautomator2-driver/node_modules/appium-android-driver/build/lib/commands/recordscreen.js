...
async function stopRecordingScreen(options = {}) {
    ...
    try {
        ...
        for (const pathOnDevice of props.records) {
            const relativePath = node_path_1.default.resolve(tmpRoot, node_path_1.default.posix.basename(pathOnDevice));
...
async function mergeScreenRecords(mediaFiles) {
    ...
    const configFile = node_path_1.default.resolve(node_path_1.default.dirname(mediaFiles[0]), 'config.txt');
    ...
    const result = node_path_1.default.resolve(node_path_1.default.dirname(mediaFiles[0]), `merge_${Math.floor(+new Date())}${DEFAULT_EXT}`);
...
async function terminateBackgroundScreenRecording(adb, force = true) {
    ...
    if (lodash_1.default.isEmpty(screenrecordPids)) {
        return false;
    }
    try {
        await adb.shell(['kill', force ? '-15' : '-2', ...screenrecordPids.map(String)]);
        await (0, asyncbox_1.waitForCondition)(async () => lodash_1.default.isEmpty(await adb.getProcessIdsByName(SCREENRECORD_BINARY)), {
