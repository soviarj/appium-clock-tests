...
function parseKnownGoodVersionsWithDownloadsJson(jsonStr) {
    ...
    try {
        ...
        throw new Error(`Storage JSON cannot be parsed. Original error: ${e instanceof Error ? e.message : String(e)}`, { cause: e });
    }
    /**
     * Example output:
     * {
    ...
    for (const { version, downloads } of json.versions) {
        ...
        for (const downloadEntry of downloads.chromedriver) {
            ...
            const key = `${node_path_1.default.basename(node_path_1.default.dirname(node_path_1.default.dirname(downloadEntry.url)))}/` +
                `${node_path_1.default.basename(downloadEntry.url)}`;
            mapping[key] = {
